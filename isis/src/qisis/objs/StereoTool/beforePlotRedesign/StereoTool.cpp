...
*/
   ...
   *
   */
  StereoTool::StereoTool(QWidget *parent) : Tool(parent) {
...
//          this, SLOT(userBaseRadius(QString)));
    connect(m_radiusLineEdit, SIGNAL(textChanged(QString)),
            this, SLOT(userBaseRadius(QString)));
  ...
  void StereoTool::showHelp() {
    ...
    QLabel *quickFunction = new QLabel(
        ...
        "you click on a cube viewprot window using one of the following "
    ...
    QLabel *quickDesc = new QLabel("<p><ul>"
      ...
      "control point is refined, click the \"Calculate Elevation\" button in "
      "the Stereo Tool window.</li>"
      "<li>Left click and drag will create an elevation profile "
      "after you create the start and end control points.  A dialog "
    ...
    QLabel *mouseLabel = new QLabel("<br><p><h3>Mouse Button Functions:</h3>");
    controlPointLayout->addWidget(mouseLabel);

    QLabel *controlPointDesc = new QLabel("<ul>"
      "<li>Left   - Edit the closest control point</li>"
      "<li>Middle - Delete the closest control point</li>"
      "<li>Right  - Create new control point at cursor location</li></ul>");
  ...
  void StereoTool::userBaseRadius(QString radius) {
    cout<<" In StereoTool::userBaseRadius"<<endl;
    //  TODO  test validity
    try {
        m_baseRadius = Distance(radius.toDouble(), Distance::Meters);
  ...
  void StereoTool::setupFiles() {
    ...
    */
    ...
    QVector< MdiCubeViewport * > linkedViewports;
    for (int i = 0; i < (int)cubeViewportList()->size(); i++) {
      if ((*(cubeViewportList()))[i]->isLinked())
        linkedViewports.push_back((*(cubeViewportList()))[i]);
    }

    if (linkedViewports.size() < 2) {
    ...
    if (linkedViewports.at(0)->cube() == m_leftCube ||
        linkedViewports.at(0)->cube() == m_rightCube) {
      if (linkedViewports.at(1)->cube() == m_leftCube ||
          linkedViewports.at(1)->cube() == m_rightCube) {
        return;
      }
    }
    ...
    if (m_controlNet) {
      if (m_controlNet->GetTarget() !=
            linkedViewports.at(0)->cube()->getCamera()->Target()) {
    ...
    // not new.
    clearFiles();
    setFiles(linkedViewports.at(0)->cube(), linkedViewports.at(1)->cube());
...
//    clearProfile();
      ...
      if (RubberBandTool::mouseButton() & Qt::RightButton) {
        ...
        try {
          ...
          if (m_startPoint->GetId() == m_endPoint->GetId()) {
            throw iException::Message(iException::User, "No End Point",
                                      _FILEINFO_);
          }
        }
        catch (iException &e) {
   ...
   *                          is clicked.
   */
  void StereoTool::createPoint(double lat, double lon, QString pointId) {
...
//    return;
    ...
    // If point Id not passed in, prompt for pointId
    bool goodId = false;
    if (!pointId.isEmpty()) {
      goodId = true;
    }
  ...
  void StereoTool::calculateElevation(ControlPoint *point) {
    ...
    if (m_radiusBox->currentText() == "DEM Radius") {
      ...
      leftCamera->SetImage((*point)[Left]->GetSample(),
                           (*point)[Left]->GetLine());
      m_baseRadius = leftCamera->DemRadius(leftCamera->GetLatitude(),
...
//      cout<<setprecision(15)<<"radius = "<<radius<<"  baseRadius = "<<m_baseRadius.meters()<<"  elevation = "<<elevation<<endl;
    ...
    // ControlMeasure to save other values.
  ...
  void StereoTool::saveElevations() {
    ...
    for (int i = 0; i < m_controlNet->GetNumPoints(); i++) {
      ...
      data = QString::fromStdString(p.GetId()) + "," +
             QString::number(apriori.GetLatitude().degrees()) + "," +
             QString::number(apriori.GetLongitude().degrees()) + "," +
             QString::number(apriori.GetLocalRadius().meters(), 'f', 6) + 
             "," + 
             QString::number(p.GetMeasure(Left)->GetFocalPlaneMeasuredX()) +
             "," +
             QString::number(p.GetMeasure(Left)->GetFocalPlaneMeasuredY()) +
             "," + leftFile + "," +
...
#if 0
  void StereoTool::updateTool() {

    MdiCubeViewport *activeViewport = cubeViewport();
    cout<<"StereoTool::updateTool"<<endl;
    if (activeViewport == NULL) {
      if (m_stereoTool->isVisible()) m_stereoTool->close();
    }
    else {
      setupFiles();
    }
...
//  (*m_endPoint)[Right]->SetCoordinate(731.82,2666.75);
    ...
    QVector<double> elevations, pixels;
    double elevation, elevationError;
...
//        AutoReg::RegisterStatus status = ar->Register();
        if (ar->Success()) {
          ...
          if (m_radiusBox->currentText() == "DEM Radius") {
            ...
            m_baseRadius = shortCube->getCamera()->DemRadius(
...
//    cout<<"Registration attempts = "<<(int)shortLength<<"  failures = "<<failureCount<<endl;
    ...
    PlotWindow *plotWindow = new PlotWindow("Elevation Profile",
                                             PlotCurve::PixelNumber,
                                             PlotCurve::Elevation,
