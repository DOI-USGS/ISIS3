...
namespace Isis {
  ...
  void RubberBandTool::enable(RubberBandTool::RubberBandMode mode,
                              bool showIndicatorColors) {
    getInstance()->enableBanding(mode, showIndicatorColors);
  }
  ...
  QRect RubberBandTool::rectangle() {
    return getInstance()->getRectangle();
  }
  ...
  QList<QPoint> RubberBandTool::getVertices() {
    return getInstance()->getFoundVertices();
  }


  RubberBandTool::RubberBandMode RubberBandTool::getMode() {
    return getInstance()->getCurrentMode();
  }


  RubberBandTool::RubberBandMode RubberBandTool::getCurrentMode() {
    return p_bandingMode;
  };


  double RubberBandTool::getArea() {
    return getInstance()->getAreaMeasure();
  }


  double RubberBandTool::getAreaMeasure() {
    return 0.0; //<! Returns the area of the figure
  }


  double RubberBandTool::getAngle() {
    return getInstance()->getAngleMeasure();
  }


  geos::geom::Geometry *RubberBandTool::geometry() {
    return getInstance()->getGeometry();
  }


  Qt::MouseButton RubberBandTool::mouseButton() {
    return getInstance()->getMouseButton();
  }


  //<! This returns true if we can return complete & valid data.
  bool RubberBandTool::isValid() {
    return getInstance()->figureValid();
  }


  bool RubberBandTool::isPoint() {
    return getInstance()->figureIsPoint();
  }


  void RubberBandTool::allowPoints(unsigned int pixTolerance) {
    getInstance()->enablePoints(pixTolerance);
  }


  void RubberBandTool::allowAllClicks() {
    getInstance()->enableAllClicks();
  }
