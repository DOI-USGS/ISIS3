 ...
 */
static map<int, bool> excludedFiles;
 ...
 */
static map<pair<int, int>, double> excludedFramelets;
 ...
 */
static vector<PvlObject> excludedDetails;
 ...
 */
static vector< vector<double> > inputFrameletAverages;
 ...
 */
static Statistics inputFrameStats;
 ...
 */
static vector<BigInt> numInputDns;
 ...
 *       field file.
 */
void makeflat(UserInterface &ui) {
   ...
   */
  PvlGroup excludedFilesGrp("ExcludedFiles");
  for(currImage = 0; currImage < inList.size(); currImage++) {
    if(Excluded(currImage)) {
      excludedFilesGrp += PvlKeyword("File", inList[currImage].original());
    }
  }

  // log the results
  Application::Log(excludedFilesGrp);

  if(ui.WasEntered("EXCLUDE")) {
    Pvl excludeFile;

    // Find excluded files
    excludeFile.addGroup(excludedFilesGrp);
