...
namespace Isis {
  ...
  double Mcewen::photometry ( double i, double e, double g, int band ) const {
    // Test for valid band
   ...
   */   
  double  Mcewen::photometry ( const Parameters &parms, double i, double e, double g ) const {
    ...
    double mu = cos(e);
    double mu0 = cos(i);
    double Lalpha = exp(parms.BETA * g + parms.GAMMA * g * g + parms.DELTA * g * g *g);
   ...
   */
  void Mcewen::init ( PvlObject &pvl, Cube &cube ) {
