...
namespace Isis {
  ...
  /**
   * @brief An implementation of the McEwen photometric function
   *  
  ...
  class Mcewen: public PhotometricFunction{
    public:
      /**
       * @brief Create McEwen photometric object
       *
       */
      Mcewen (PvlObject &pvl, Cube &cube, bool useCamera) : PhotometricFunction(pvl, cube, useCamera) {init(pvl, cube);};
    ...
    private:
       ...
       */
      struct Parameters {
        Parameters() : AMC(0.0), BETA(0.0), GAMMA(0.0), DELTA(0.0), wavelength(0.0), tolerance(0.0),
                       units("Degrees"), phaUnit(1.0), band(0), phoStd(0.0),
                       iProfile(-1) { }
        ~Parameters() { }
        bool IsValid() const { return (iProfile != -1); }
        double AMC, BETA, GAMMA, DELTA;  //<! McEwen parameters
