...
namespace Isis {
   ...
   * models from it. The cube is needed to match all potential profiles for each band. 
   *
   ...
   * @author 2016-10-07 Driss Takir
   */
  double Rolo::photometry(const Parameters &parms, 
    ...
    //  surge term.
    double rcal = (mu0 / (mu + mu0))
                  * (parms.C0 * exp(-parms.C1 * phaseAngle)
                     + parms.A0 
                     + (parms.A1 * phaseAngle) 
                     + (parms.A2 * pow(phaseAngle, 2)) 
                     + (parms.A3 * pow(phaseAngle, 3)) 
                     + (parms.A4 * pow(phaseAngle, 4)));
    return rcal;
  } 
  ...
  void Rolo::report(PvlContainer &pvl) {
    ...
    for (unsigned int i = 0; i < m_bandpho.size(); i++) {
      ...
      C0.addValue(toString(p.C0));
      C1.addValue(toString(p.C1));
   ...
   *
   * @return Rolo::Parameters Container of valid values.  If not
   *         found, a value of iProfile = -1 is returned.
   *
   * @author 2016-10-07 Driss Takir
  */
  Rolo::Parameters Rolo::findParameters(const double wavelength) const {
   ...
   *
   * @return Rolo::Parameters Container of extracted values
   *
   * @author 2016-10-07 Driss Takir
   */
  Rolo::Parameters Rolo::extract(const DbProfile &p) const {

    Parameters pars;
    pars.C0 = toDouble(ConfKey(p, "C0", toString(0.0)));
    pars.C1 = toDouble(ConfKey(p, "C1", toString(0.0)));
