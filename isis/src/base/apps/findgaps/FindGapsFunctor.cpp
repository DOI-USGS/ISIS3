...
namespace Isis {
  ...
  void FindGapsFunctor::operator()(Buffer &in, Buffer &out) const {
    ...
    for (int i = 0; i < m_gaps->groups(); i++) {
      ...
      int gapBand = IString::ToInteger((*m_gaps).group(i).findKeyword(" NewGapInBand")[0]);
      int gapStart = IString::ToInteger((*m_gaps).group(i).findKeyword("StartLine")[0]);
      int gapEnd = IString::ToInteger((*m_gaps).group(i).findKeyword("LastGapLine")[0]);
  ...
  void FindGapsFunctor::addGapToGroup() const {
    ...
    */
    if (IString::ToInteger(m_gap->findKeyword("StartLine")[0]) <
        IString::ToInteger(m_gap->findKeyword("LastGapLine")[0])) {
