...
namespace Isis {
   ...
   */
  LocalKernels::LocalKernels(const std::string &filename) {
    Pvl pvl(filename);
    Init(pvl);
  } 
   ...
   */
  LocalKernels::LocalKernels(Cube &cube) { 
    Init(*cube.Label());
  } 
   ...
   */
  LocalKernels::LocalKernels(Pvl &pvl) { 
    Init(pvl);
  } 
   ...
   */
  void LocalKernels::Init(Pvl &pvl) {
    UnLoad();
    _kernels.clear();
  ...
  int LocalKernels::Load(const std::string &ktypes) {
  ...
  bool LocalKernels::Load(LocalKernels::KernelFile &kfile) {
   ...
   * @brief Unloads all kernels if they were loaded when found
   */
  int LocalKernels::UnLoad(const std::string &ktypes) {
    //  If not types specified, return them all
    int nUnloaded(0);
    if (ktypes.empty()) { 
      nUnloaded = UnLoad();     
   ...
   * @brief Unloads all kernels if they were loaded when found
   */
  bool LocalKernels::UnLoad(KernelFile &kfile) {
 ...
 /**
   ...
   */
  std::vector<std::string> LocalKernels::getList(bool removePath) const {
    vector<string> flist;
    for (unsigned int i = 0 ; i < _kernels.size() ; i++) {
      if (removePath) {
   ...
   * @return std::vector<std::string> A vector of filenames of SPICE kernels
   */
  std::vector<std::string> LocalKernels::getList(const std::string &ktypes, 
  ...
  std::vector<std::string> LocalKernels::getTypes(const std::string &ktypes) const {
   // Find types and return requested types
    vector<string> tlist;
    iString::Split(',', ktypes, tlist);
   ...
   */
  void LocalKernels::addKernels(const KernelList &klist) {
   ...
   * 
   * @return LocalKernels::KernelList List of scrutinized kernel file names 
   */
  LocalKernels::KernelList LocalKernels::findKernels(Pvl &pvl, 
   ...
   */
  bool LocalKernels::IsNaifType(const std::string &ktype) const {
   ...
   * 
   * @return LocalKernels::KernelFile An internal Kernels file structure describing 
   *         the file.
   */
  LocalKernels::KernelFile LocalKernels::examine(const std::string &kfile) const {
   ...
   *         "UNKNOWN" is returned.
   */
  std::string LocalKernels::resolveType(const std::string &kfile) const {
    Filename kernFile(kfile);
   ...
   */
  int LocalKernels::getCameraVersion(Pvl &pvl) const {
    PvlGroup &kernels = pvl.FindGroup("Kernels",Pvl::Traverse);
    int cv(0);
    // Check for the keyword
