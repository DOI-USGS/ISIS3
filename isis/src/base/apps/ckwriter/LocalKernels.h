#if !defined(LocalKernels_h)
#define LocalKernels_h
/**                                                                       
 * @file                                                                  
 * $Revision: 1.1 $
...
namespace Isis {
   ...
   * @history 2011-05-29 Debbie A. Cook Changed name to LocalKernels to avoid
   *                                confusion with system Kernels class 
   */
class LocalKernels {
    public:
      /** Default Constructor */
      LocalKernels() { }
      LocalKernels(const std::string &filename);
      LocalKernels(Cube &cube);
      LocalKernels(Pvl &pvl);
      /** Destructor always unloads the kernels from the pool */
      virtual ~LocalKernels() { UnLoad(); }
