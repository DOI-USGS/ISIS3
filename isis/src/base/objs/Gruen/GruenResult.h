...
namespace Isis {
  ...
  struct GruenResult {
    ...
    inline const GSLVector &Alpha() const {
      return (alpha);
    }
    ...
    inline double SampleError() const {
      return (_startSamp - _finalSamp);
    }

    /** Return line error in solution */
    inline double LineError() const {
      return (_startLine - _finalLine);
    }
    ...
    inline double SampleUncertainty() const {
      return (skmat[0][0]);
    }
    /** Line shift error as determined by Jacobi solution */
    inline double LineUncertainty() const {
      return (skmat[1][1]);
    }
    ...
    inline Affine getChipTransform() const {
      return (_chipTransform);
    }
