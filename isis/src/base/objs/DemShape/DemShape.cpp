...
namespace Isis {
  ...
  /**
   * Find the intersection point with the DEM. The intersection point must be in
     front of the observer. Start by intersecting with a nearby horizontal
     surface, then refine using the secant method. This was validated to work
     with ground-level sensors. Likely can do well with images containing a
     limb.
   *
   * @param observerPos
   * @param lookDirection
  ...
  bool DemShape::intersectSurface(vector<double> observerPos,
    ...
    // The intersection point must be ahead of the observer
    if (t0 < 0.0)
      converged = false;
