...
//   p_quaternion.Set ( p_CJ );
  ...
  void SpiceRotation::LoadCache(Table &table) {
    ...
    // coefficient table for angle1, angle2, and angle3
      ...
      if (degree == 0  && p_cacheAv.size() > 0) p_hasAngularVelocity = true; //FIXME
    }
    else  {
  ...
  void SpiceRotation::ReloadCache() {
    ...
    // Clear existing matrices from cache
      ...
      //  nothing.
      if (p_degree > 0 && p_cacheAv.size() > 1) { // FIXME? 
        p_cacheAv.clear(); // remove 
      }
 ...
 // End section added 06-20-2015 DAC
  ...
  std::vector<double> SpiceRotation::J2000Vector(const std::vector<double> &rVec) {
    ...
    if (rVec.size() == 3) {
      double TJ[3][3];
      mxm_c((SpiceDouble *) &p_TC[0], (SpiceDouble *) &p_CJ[0], TJ); // FIXME
      jVec.resize(3);
...
//            int startnoff = start1off + nints - 1;
  ...
  std::vector<double> SpiceRotation::Matrix() {
    ...
    TJ.resize(9);
    mxm_c((SpiceDouble *) &p_TC[0], (SpiceDouble *) &p_CJ[0], (SpiceDouble( *) [3]) &TJ[0]); // FIXME
    NaifStatus::CheckErrors();
    return TJ;
  }
  ...
  std::vector<double> SpiceRotation::ConstantRotation() {
    ...
    std::vector<double> q;
    q.resize(4);
    m2q_c((SpiceDouble( *)[3]) &p_TC[0], &q[0]); // FIXME
    NaifStatus::CheckErrors();
    return q;
  }
...
/*      std::vector<double>::iterator pos;
      ...
      // do I need to do any of the above? FIXME
