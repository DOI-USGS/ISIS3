...
int main() {
  try {
   ...
   // Test time-shift SetImage 
    ...
    pixRes2 /= 10000000;
    pvl.findGroup("Mapping")["PixelResolution"] = std::to_string(pixRes2);
    ...
    minLat = round(minLat);
    minLat /= 100;
    camMap.findGroup("Mapping")["MinimumLatitude"] = std::to_string(minLat);

    double pixRes = camMap.findGroup("Mapping")["PixelResolution"];
    pixRes *= 100;
    pixRes = round(pixRes);
    pixRes /= 100;
    camMap.findGroup("Mapping")["PixelResolution"] = std::to_string(pixRes);

    double minLon = camMap.findGroup("Mapping")["MinimumLongitude"];
    minLon *= 100000000000.0;
    minLon = round(minLon);
    minLon /= 100000000000.0;
    camMap.findGroup("Mapping")["MinimumLongitude"] = std::to_string(minLon);
