...
namespace Isis {
   ...
   * @history 2018-07-29 UA/OSIRIS-REx IPWG Team  - Fixed errors in 
   *                          importGeometry() where an exception was being thrown
   *                          when checking for valid Geometry. It now checks for
   *                          this case and runs buffer(0) to attempt to fix the
   *                          geometry. This is typically a self-intersection
   *                          error that can be repaired with a buffer(0)
   *                          operator.
   */
