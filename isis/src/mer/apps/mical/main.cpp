...
//  gbl::mi = new Mer::MiCalibration(*(pack->GetCube()), calKernel);
  ...
  if(!ui.GetBoolean("ZECORRECTION") || gbl::mi->ShutterEffectCorrectionFlag() == "TRUE") {
    ...
    calgrp += PvlKeyword("ZeroExposureValue", Isis::toString(gbl::mi->ZeroExposureValue()));
  ...
  if(!ui.GetBoolean("AACORRECTION")) {
    ...
    calgrp += PvlKeyword("ActiveAreaValue", Isis::toString(gbl::mi->ActiveAreaValue()));
  ...
  calgrp += PvlKeyword("DarkCurrentFullModel", Isis::toString(fullModel));

  //Add temperature values to the radiometry group
  calgrp += PvlKeyword("CCDTemperture", Isis::toString(gbl::mi->CCDTemperatureCorrect()));
  calgrp += PvlKeyword("PCBTemperature", Isis::toString(gbl::mi->PCBTemperature()));
  if(stagestop == "IOF") {
    calgrp += PvlKeyword("OmegaNaught", Isis::toString(gbl::mi->OmegaNaught()));
    calgrp += PvlKeyword("SunAU", Isis::toString(gbl::sunAU));
  }
