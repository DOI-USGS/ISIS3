...
typedef struct TRANS2d3p  //2d 3 parameter transformation
{
  ...
  double limit[2];  //where to stop using this transformation, will eventually be a point in the coordinate system of the new cube, a vertical line through this point will be the dividing line
}TRANS2d3p;
...
void IsisMain() {
  ...
  std::vector <TRANS2d3p> solV;  //vector of possible solutions to a transformation-all transformations resulting from any possible combination of minimal sets of data are calculated to find the best solution  in an outlier resistant way
  ...
  double  resolution = ui.GetDouble("MICRONS"),    //pixel size in microns
          ...
          nominalTrans     = TRANS_N*5.0/resolution,  //nominal sample direction overlap between successive sub scans
          averageLines     = AVERl  *5.0/resolution;  //scaled average diference between the top and bottom fiducials

  if( 10.0/resolution < 0.5) play=0.5;
  ...
  patternS.Write("PatternTemp.cub");  //debug
  ...
  *arS->PatternChip()   = patternS;  //patternS chip is constant
  ...
  for (i=0;i<7;i++) {
    ...
    for (s= scanFid[0][0], l=scanFid[0][1];s + nominalTrans < scanS;s+=averageSamples/2.0) {
      ...
      *arS->SearchChip()   = searchS;
      ...
      /*else {  //DEBUG whole else block
        if (regStatus == AutoReg::SuccessPixel) printf("DEBUG: initialPoint: %lf %lf\n",arS->CubeSample(),arS->CubeLine());
        else printf("DEBUG: initialPoint: %lf %lf\n",s,l+averageLines);
        selectionChip.Write("selectionChip.cub");
        getchar();
      }*/

      //look for the top fiducial
      if (s == scanFid[0][0])continue;  //if this is the first time through the loop then the top fiducial was already found
      searchS.TackCube(s,l);
      searchS.Load(*panC[i],0,scale);
      *arS->SearchChip()   = searchS;
      regStatus = arS->Register();
      ...
      if (centroid.elipticalReduction(&selectionChip,95,play,2000) != 0) {  //elliptical trimming/smoothing... if this fails move on
        ...
        l=fid[1] = inputChip.CubeLine();        //s and l are refined here to help follow any trends in the image/film
        scanFid.push_back(fid);
      }
      /*else {  //DEBUG whole else block
        if (regStatus == AutoReg::SuccessPixel) printf("DEBUG: initialPoint: %lf %lf\n",arS->CubeSample(),arS->CubeLine());
      ...
      //printf("DEBUG4  s: %lf l: %lf nominalTrans: %lf scanS: %lf\n",s,l,nominalTrans,scanS);
    }

    printf("DEBUG4 fidmarks: %d\n",int(scanFid.size()));
    ...
    for (j=scanFid.size()-1;j>=0;j--) {
      ...
      *arS->SearchChip()   = searchS;
      regStatus = arS->Register();
      if (regStatus == AutoReg::SuccessPixel) {  //if autoreg is successful a smaller window can be used for the following steps
    ...
    printf("DEBUG5 conjugate fiducial marks: %d\n",int(conFid.size()));
    //STEP 3. robustly calculate the transformation (that is in a manner resistent to erroneous measurements)
    //calculate the transformation from i+1 to i
    //RANSAC2 algorithim
      //Find two point solutions and then attempt to generalize to more points
      //this helps to identify measurement blunders by seeing if certain points are excluded consistently--or equivalently if certain solutions keep repeating themselves
      //the limitation in this case is that there will never be more than 6 points, if the number of points drops to <=three blunder detection becomes impossible
  ...
  delta = inverse(ata)*atw  iterate until corrections in delta are insignificant
    */
    ...
    solV.clear();
    //printf("DEBUG6\n");
    double wdot[2],adot[2][3],ata[6],atf[3];  //matrix names loosly following the naming convention of Uotilia see large comment above

    int l,m,n,o,ni;  //additonal indeces for matrix multipilcations, etc

    //printf("DEBUG: conFid.size(): %d\n",conFid.size());
    ...
    for (j=0;j< int(conFid.size())-1;j++) {
      for (k=j+1;k<int(conFid.size());k++) {
        ...
        sol.theta = atan2( conFid[j][3] - conFid[k][3] , conFid[j][2] - conFid[k][2] ) - atan2( conFid[j][1] - conFid[k][1] , conFid[j][0] - conFid[k][0] ) ;
        ...
        for (ni=0;ni<50;ni++) {    //iteration limit is imposed here, apon successful convergance the code breaks out of this for loop
          ...
          ct = cos(sol.theta);  //cosine and sin computed only once
          st = sin(sol.theta);
          ...
          for (m=0;m<int(conFid.size());m++) {  //for each point
            ...
            //add transpose(adot)*wdot to atf  //note: becuase m is constant, and every weight is equal, it is irrelevant and ignored
          ...
          if (choleski_solve(ata,atf,3,2) != 1) continue;  //if the solution fails move on to the next two point solution  //calculation is done in place as delta is returned in atf
          ...
          if (fabs(atf[0]) < 1e-10 && fabs(atf[0]) < 1e-5 && fabs(atf[0]) < 1e-5) {    //solution converged
            //find residual stats--Note residuals are caluclated for all points-including any that might have been excluded from thes solution above
            for (m=0,sol.maxR=0.0,sol.averR=0.0,sol.limit[0] =0.0,sol.limit[1] =0.0;m<int(conFid.size());m++) {  //for each point
              wdot[0] = -( conFid[m][0]*ct - conFid[m][1]*st + sol.dx - conFid[m][2]);
              wdot[1] = -( conFid[m][0]*st + conFid[m][1]*ct + sol.dy - conFid[m][3]);
  ...
  for (i=0;i<7;i++) {
    for (j=i+1;j<8;j++) {
      ...
      trans[i].theta = atan2(cos(trans[j].theta)*sin(trans[i].theta) + sin(trans[j].theta)*cos(trans[i].theta),cos(trans[j].theta)*cos(trans[i].theta) - sin(trans[j].theta)*sin(trans[i].theta));

      temp        = cos(trans[j].theta)*trans[i].dx - sin(trans[j].theta)*trans[i].dy + trans[j].dx;
      trans[i].dy = sin(trans[j].theta)*trans[i].dx + cos(trans[j].theta)*trans[i].dy + trans[j].dy;
  ...
  //trans[i].limit was previous calculated as the center of mass of the conjugate fiducials in the scani system
  //transfrom it now into the stiched cube system to be used as a limit between scans
  for (i=0;i<7;i++) {
    temp       = trans[i].limit[0]*cos(trans[i].theta) - trans[i].limit[1]*sin(trans[i].theta) + trans[i].dx;
    trans[i].limit[1] = trans[i].limit[0]*sin(trans[i].theta) + trans[i].limit[1]*cos(trans[i].theta) + trans[i].dy;
  ...
  for (i=0;i<8;i++) {
    ...
    trans[i].dx += -sampleFrom+1;  //a shift so that the transform puts sampleFrom at sample 1 of the rubber sheeted sub cubes
