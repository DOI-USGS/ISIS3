...
namespace Isis {
    ...
    void Exponential::report ( PvlContainer &pvl ) {
        ...
        for (unsigned int i = 0; i < m_bandpho[0].aTerms.size(); i++)
            aTermKeywords.push_back(PvlKeyword("A" + Isis::toString((int) i)));
        for (unsigned int i = 0; i < m_bandpho[0].bTerms.size(); i++)
            bTermKeywords.push_back(PvlKeyword("B" + Isis::toString((int) i)));
    ...
    Exponential::Parameters Exponential::extract ( const DbProfile &p ) const {
        ...
        for (int i = 0; i < p.size(); i++) {
            if (p.exists("A" + QString::number(i)) || p.exists("B" + QString::number(i))) {
                pars.aTerms.push_back(ConfKey(p, "A" + QString::number(i), (QString)"1.0").toDouble());
                pars.bTerms.push_back(ConfKey(p, "B" + QString::number(i), (QString)"0.0").toDouble());
            }
        }
        ...
        pars.units = ConfKey(p, "ExponentialUnits", QString("Radians"));
