...
TEST_F(MroCtxCube, FunctionalTestCtxcalDefault) {
  ...
  EXPECT_NEAR((double)radGroup.findKeyword("iof"), 1.8676443085546e-04, 1e-12);
...
TEST_F(MroCtxCube, FunctionalTestCtxcalFlatfile) {
  ...
  EXPECT_DOUBLE_EQ(oCubeStats->ValidPixels(), 400);
  EXPECT_NEAR(oCubeStats->StandardDeviation(), 0.0018248585597074806, 1e-14);
}
...
TEST_F(MroCtxCube, FunctionalTestCtxcalCameraComparison) {
  ...
  EXPECT_NEAR((double)noCamLab->findObject("IsisCube").findGroup("Radiometry").findKeyword("iof"), 1.8676443085546e-04, 1e-12);
}
