...
TEST_F(LineScannerCube, FunctionalTestNoprojLineScanner) {
  ...
  std::unique_ptr<Histogram> hist (oCube.histogram(1));
  EXPECT_NEAR(hist->Average(), 26.259685166498699, .000001);
  EXPECT_NEAR(hist->Sum(), 78070.044000000635, .0001);
  EXPECT_EQ(hist->ValidPixels(), 2973);
  EXPECT_NEAR(hist->StandardDeviation(), 11.938131126485441, .0001);
}
