...
static QString APP_XML = FileName("$ISISROOT/bin/xml/shadowcamcal.xml").expanded();

TEST(Shadowcamcal, ShadowcamcalTestDefault) {
  QTemporaryDir prefix;
  QString cubeFileName = prefix.path() + "/ShadowcamcalTEMP.cub";
  QVector<QString> args = {"from=data/shadowcam/M013049982SE.2isis.crop.cub",
                           "to=" + cubeFileName };

  UserInterface options(APP_XML, args);
  try {
    shadowcamcal(options);
  }
  catch (IException &e) {
  ...
  // Dimensions Group
  EXPECT_EQ(cube.sampleCount(), 3072);
  ...
  EXPECT_NEAR(hist->Average(), 0.075320602232901976, .0001);
  EXPECT_DOUBLE_EQ(hist->Sum(), 231.38489005947486);
  EXPECT_EQ(hist->ValidPixels(), 3072);
  EXPECT_NEAR(hist->StandardDeviation(), 0.20249537390113267, .00001);
}

TEST(Shadowcamcal, ShadowcamcalTestCalibrated) {
  QTemporaryDir prefix;
  QString cubeFileName = prefix.path() + "/ShadowcamcalTEMP.cub";
  ...
  try {
    ...
    EXPECT_THAT(e.what(), HasSubstr("is calibrated, no need to run shadowcamcal."));
  }
}