...
TEST_F(TempTestingFiles, DISABLED_FunctionalTestJitterfitDefault){
  ...
  EXPECT_DOUBLE_EQ(Isis::toDouble(inst["JitterSampleCoefficients"][0]), -6.3185740156776303e-09);
  EXPECT_DOUBLE_EQ(Isis::toDouble(inst["JitterSampleCoefficients"][1]), 2.44034713747508e-06);
  EXPECT_DOUBLE_EQ(Isis::toDouble(inst["JitterSampleCoefficients"][2]), 0.0032581267520383002);
  EXPECT_DOUBLE_EQ(Isis::toDouble(inst["JitterLineCoefficients"][0]), -2.7623177125388001e-09);
  EXPECT_DOUBLE_EQ(Isis::toDouble(inst["JitterLineCoefficients"][1]), 3.3727910440682698e-06);
  EXPECT_DOUBLE_EQ(Isis::toDouble(inst["JitterLineCoefficients"][2]), 0.0013484324974775001);
  ...
  Pvl stats = Pvl(outputRegistrationStats.toStdString());
