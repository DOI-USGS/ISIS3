...
TEST_F(DemCube, FunctionalTestShadowTime) {
  ...
  PvlGroup shadowStats = appLog.findGroup("ShadowStatistics");
  EXPECT_EQ(int(shadowStats["NumComputedAzimuthElevations"]), 10000);
  EXPECT_NEAR(double(shadowStats["AverageAzimuth"]), 141.60048549076001, 1e-6);
  EXPECT_NEAR(double(shadowStats["MinimumAzimuth"]), 141.18641700737001, 1e-6);
  EXPECT_NEAR(double(shadowStats["MaximumAzimuth"]), 142.02798328761, 1e-6);

  EXPECT_NEAR(double(shadowStats["AverageElevation"]), 54.723733961424003, 1e-7);
  EXPECT_NEAR(double(shadowStats["MinimumElevation"]), 54.185416345476, 1e-7);
  EXPECT_NEAR(double(shadowStats["MaximumElevation"]), 55.26088378675, 1e-7);
  ...
  EXPECT_NEAR(hist->Average(), 0.57757151580484289, 1e-9);
  EXPECT_NEAR(hist->Sum(), 5486.9294001460075, 1e-5);
  ASSERT_EQ(hist->ValidPixels(), 9500);
  EXPECT_NEAR(hist->StandardDeviation(), 0.0027113946106837737, 1e-10);
}
